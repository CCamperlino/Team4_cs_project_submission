...
// ====== SAVE & LOAD ======
void saveData() {
    ofstream itemOut("inventory.txt");
    for (const auto& item : inventory) {
        itemOut << item.id << "," << item.name << "," << item.quantity << ","
                << item.reorderPoint << "," << item.locationId << "\n";
    }

    ofstream locOut("locations.txt");
    for (const auto& loc : locations) {
        locOut << loc.id << "," << loc.description << "\n";
    }

    ofstream dmgOut("damaged.txt");
    for (const auto& dmg : damagedItems) {
        dmgOut << dmg.itemId << "," << dmg.quantity << "\n";
    }
...
void loadData() {
    ...
    ifstream itemIn("inventory.txt");
    string line;
    while (getline(itemIn, line)) {
        Item item;
        size_t pos = 0;
        ...
        if (tokens.size() == 5) {
            ...
            item.reorderPoint = stoi(tokens[3]);
            item.locationId = tokens[4];
            inventory.push_back(item);
        }
    }

    ifstream locIn("locations.txt");
    while (getline(locIn, line)) {
        size_t pos = line.find(',');
        if (pos != string::npos) {
            Location loc;
            loc.id = line.substr(0, pos);
            loc.description = line.substr(pos + 1);
            locations.push_back(loc);
        }
    }

    ifstream dmgIn("damaged.txt");
    while (getline(dmgIn, line)) {
        size_t pos = line.find(',');
        if (pos != string::npos) {
            DamagedItem dmg;
            dmg.itemId = line.substr(0, pos);
            dmg.quantity = stoi(line.substr(pos + 1));
            damagedItems.push_back(dmg);
        }
    }
...
void DataChoice() {
    int UserDataChoice;
    cout << "Choose if you would like to save or load data. \n";
    cout << "1. Save Data \n";
    cout << "2. Load Data \n";
    cin >> UserDataChoice;
